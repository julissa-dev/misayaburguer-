...
@section('contenido')
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Editar Producto: {{ $producto->nombre }}</h2>
        ...
        <form action="{{ route('admin.productos.update', $producto) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="number" name="precio" id="precio" value="{{ old('precio', $producto->precio) }}" step="0.01" min="0"
                           class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                ...
                <div>
                    ...
                    @if($producto->imagen_url)
                        <div class="flex items-center space-x-4 mb-2">
                            <img src="{{ asset('storage/img/productos/' . $producto->imagen_url) }}" alt="Imagen Actual" class="w-16 h-16 object-cover rounded-md border border-gray-300 shadow-sm">
                            <span class="text-sm text-gray-600">{{ $producto->imagen_url }}</span>
                            <div class="flex items-center">
                                ...
                                <label for="remove_imagen" class="ml-2 block text-sm font-medium text-red-700">Eliminar imagen actual</label>
                            </div>
                        </div>
                        <p class="text-sm text-gray-500 mb-2">Sube una nueva imagen para reemplazar la actual.</p>
                    @else
                        <p class="text-sm text-gray-500 mb-2">No hay imagen cargada actualmente para este producto.</p>
                    @endif
            ...
            <div class="mb-6">
                ...
                <textarea name="descripcion" id="descripcion" rows="4"
                          class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                          placeholder="Una breve descripción del producto...">{{ old('descripcion', $producto->descripcion) }}</textarea>
                @error('descripcion')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
