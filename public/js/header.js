...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (userDropdownToggle && userDropdownMenu) {
        userDropdownToggle.addEventListener("click", function (event) {
            ...
            const deliveryDropdownMenu = document.getElementById(
                "deliveryDropdownMenu"
            );
            const cartDropdown = document.getElementById("cartDropdown");
            if (deliveryDropdownMenu)
    ...
    window.updateCartUI = function (response) {
        if (
            ...
            Swal.fire(
                "Error",
                "No se pudo actualizar el carrito. Datos incompletos.",
                "error"
            );
        }
    };
    ...
    if (searchInput && searchResultsDiv) {
        searchInput.addEventListener("input", function () {
            ...
            if (query.length > 0) {
                searchTimeout = setTimeout(async () => {
                    try {
                        const response = await fetch(
                            `/api/productos/buscar?query=${encodeURIComponent(
                                query
                            )}`,
                            {
                                headers: {
        ...
        function displaySearchResults(products) {
            ...
            <div class="search-result-item">
                <img src="${
                    product.imagen_url || "img/placeholder.png"
                }" alt="${product.nombre}">
                <div class="details">
        ...
        searchResultsDiv.addEventListener("click", async function (event) {
            ...
            if (target.classList.contains("btn-add-to-cart-search")) {
                ...
                try {
                    ...
                    console.error(
                        "Error al añadir al carrito desde la búsqueda:",
                        error
                    );
                    Swal.fire(
                        "Error",
    ...
    if (deliveryToggle && deliveryDropdownMenu && closeDeliveryBtn) {
        ...
        document.addEventListener("click", function (event) {
            if (
                !deliveryDropdownMenu.contains(event.target) &&
                !deliveryToggle.contains(event.target)
            ) {
